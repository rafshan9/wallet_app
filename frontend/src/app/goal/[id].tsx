    ...
    return (
        <View className="flex-1 bg-very_dark_blue pt-16 px-6">
            ...
            <View className="flex-row items-center justify-between mb-8">
                ...
                <Text className="text-2xl text-white font-inter_bold mx-4 flex-1" numberOfLines={1}>{goal.name}</Text>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <View className="p-6 bg-maroon rounded-3xl mb-8 border-2 border-dashed border-very_dark_blue shadow-sm">
                    <Text className="text-center text-sm font-inter_medium text-white mb-2 uppercase tracking-widest">
                        Current Balance
                    </Text>
                    <Text className="text-center text-5xl font-inter_bold text-white mb-8">
                        ${saved.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </Text>

                    <View className="mb-2 flex-row justify-between">
                        <Text className="font-inter_bold text-white">Progress</Text>
                        <Text className="font-inter_bold text-white">{progress.toFixed(0)}%</Text>
                    </View>
                    ...
                    <View className="flex-row justify-between">
                        <Text className="text-sm font-inter_medium text-white">Target: ${Number(goal.targetAmount).toLocaleString()}</Text>
                        {goal.deadline && (
                            <Text className="text-sm font-inter_medium text-white">
                ...
                <Text className="text-xl font-inter_bold mb-4 text-white">Contribution History</Text>

                {contributions.length === 0 ? (
                    <Text className="text-center text-gray-400 font-inter_medium py-8">No contributions yet.</Text>
                ) : (
